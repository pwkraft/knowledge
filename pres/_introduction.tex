...
% INTRODUCTION
% Work on transition for discrusive sophistication

\begin{frame}{Discursive Sophistication\visible<3->{ -- Structural Topic Model}}\label{overview_stm}
\begin{center}
        \begin{tikzpicture}
            \node<1-3>[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{fig/lda_empty.pdf}};
            \node<2>[align=left] (image) at (8.4,4) {
                        \emph{Attributes of Sophistication}:\\
                        \emph{\faCaretRight} Political Belief System\\
                        \emph{\faCaretRight} Accuracy motivation};
            \node<3>[align=left] (image) at (9,4) {
            ...
            \node<4>[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=\textwidth]{fig/lda.pdf}};
        \end{tikzpicture}
    \end{center}
...
\begin{enumerate}
\item \hyperlink{elaboration}{\emph{Considerations:}} How many topics are mentioned?
\item<3-> \hyperlink{eloquence}{\emph{Word Choice:}} Are terms highly descriptive of topics? 
\item<5-> \hyperlink{opinionation}{\emph{Opinionation:}} Does a respondent discuss all questions?
\end{enumerate}}
...
\begin{center}
        \begin{tikzpicture}
            \node<1>[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[height=.85\textheight]{fig/corplot_components0.pdf}};
            \node<2->[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[height=.85\textheight]{fig/corplot_components.pdf}};
            \node<3>[align=center,draw opacity=0,fill opacity=0.8,text opacity=1, white, fill=beamer@sbred] at (image.center) {Discursive Sophistication\\=\\ $\tfrac{1}{3}$(Considerations + Word Choice + Opinionation)};
        \end{tikzpicture}
    \end{center}
%\visible<2>{\flushright\small \cite{roberts2014structural}}
%\flushright{\footnotesize\texttt{\textcolor{gray}{\hyperlink{components}{[back]}}}}
