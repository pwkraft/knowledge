...
Consider again the example of multiple open-ended questions about candidate and party preferences in a survey. In such a scenario, we can characterize elaborate reasoning by leveraging variation between individuals on three levels---response behavior across different questions, the content and topics in a response, and the way each topic itself is discussed. For each dimension, I propose the following assumptions about how sophistication is manifested in verbatim responses.

First, we consider response variation across survey items. Individuals who engage in elaborate processing should hold opinions about each political actor or policy that they are asked to discuss. Given a set of multiple open-ended probes focusing on different targets of evaluation, a sophisticated respondent should be able to express their attitudes towards each question in terms of both approval or disapproval.

% 1) Distribution between answers
\begin{assumption}[Opinionation]
Sophisticated respondents hold various opinions and are therefore able to respond to each query presented in a survey.
\end{assumption}

Next, the level of elaboration should be related to the content and topics raised by each individual. Specifically, it should affect the number of distinct considerations that are raised when answering a set of open-ended questions. Sophisticated individuals should be able to elaborate more on the nature of their political attitudes for each target.

% 2) Distribution across topics/considerations
\begin{assumption}[Considerations]
Sophisticated respondents evaluate political actors and issues on multiple dimensions and are therefore able to recall a large number of considerations in response to each query presented in a survey. 
\end{assumption}
...
Talk about each topic in a reasonable way! How you talk about each consideration.
\end{assumption}
