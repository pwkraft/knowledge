...
I begin by describing the measurement approach in more detail based on the 2012 and 2016 ANES data. The same measures are used for the remaining datasets and additional information for them is provided in the appendix.

In order to measure discursive sophistication in these open-ended responses, we need to quantify the individual-level variation on each of the three dimensions discussed previously. With regard to response patterns across questions, high levels of elaboration have been argued to display a greater level of \textit{opinionation}, which is based on the degree to which individuals are able to respond to each query presented in a survey more or less equally. I quantify this diversity in relative lengths for each open-ended response as the Shannon entropy across survey items:
\begin{equation}
...
The last component of discursive sophistication addresses the way individuals talk about each consideration. More specifically, I argue that when talking about a specific topic, sophisticated respondents should use words that are clearly associated with that topic. We can use the assignments based on the structural topic model for each response to summarize the extent to which word choice tends to consist of highly descriptive terms for each topic or not. Highly descriptive word choice is thereby conceptualized as the sum of term likelihoods $P(w|t^*)$ given topic assignments over the entire set of words in $\mathcal{W}_i$:
\begin{equation}
...
All three components are positively correlated.\footnote{Also see appendix.} Focusing on the data from the ANES, all three components load on a single factor with factor loadings $>.5$
